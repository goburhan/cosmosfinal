...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice ,steemBalance,BSTEEMprice, ethereum, account }) => {
  ...
  const farmImage = 'cake-bnb'
    console.log(farm)

   const totalValue: BigNumber = useMemo(() => {

    if (farm.delegate) {
      return cakePrice.multipliedBy(steemBalance)
    }
     return farm.lpTotalInQuoteToken
     
   }, [farm.lpTotalInQuoteToken,cakePrice ,steemBalance,farm.delegate])

   const totalValueFormated = totalValue
     ? `$${Number(totalValue).toLocaleString(undefined, { maximumFractionDigits: 0 })}`
     : '-'
  const lpLabel = "CAKE-BNB"
  const earnLabel = 'CAKE'
  const farmAPY =
    farm.apy &&
    ...
    <FCard>
      ...
      <ExpandingWrapper expanded={showExpandableSection}>
        <DetailsSection
          ...
          bscScanAddress={
            farm.isTokenOnly
              ? `https://bscscan.com/token/0x3c886500D042DcCA8AA16d5d8fC4D8C1AE9dfE6f`
              : `https://bscscan.com/token/0x3c886500D042DcCA8AA16d5d8fC4D8C1AE9dfE6f`
          }
